# Test suite for read_[format] function
#
# Tests cover:
...
# - Adapt the create_test_files functions
# - Specify parameters
# - Replace all read_[format] with the appropriate function
...
test_that("read_[format] validates file existence", {
  expect_error(
    read_[format](path_nonexistent)
  )
})

test_that("read_[format] rejects incorrect file format", {
  expect_error(
    read_[format](path_wrong_format),
    error_wrong_format
  )
})
...
test_that("read_[format] successfully imports valid data", {
  result <- read_[format](path_valid)
...
test_that("read_[format] has required columns", {
  result <- read_[format](path_valid)
...
test_that("read_[format] renames columns correctly", {
  result <- read_[format](path_valid)
...
test_that("read_[format] removes unnecessary columns", {
  result <- read_[format](path_valid)
...
test_that("read_[format] converts data types correctly", {
  result <- read_[format](path_valid)
...
test_that("read_[format] sets correct source metadata", {
  result <- read_[format](path_valid)
...
test_that("read_[format] sets correct unit metadata", {
  result <- read_[format](path_valid)
...
test_that("read_[format] sets correct coordinate system", {
  result <- read_[format](path_valid)
...
test_that("read_[format] sets filename in metadata", {
  result <- read_[format](path_valid)
...
test_that("read_[format] handles timestamps when present", {
  result <- read_[format](path_with_timestamps)

  meta <- aniframe::get_metadata(result)
  expect_true("start_datetime" %in% names(meta))
  expect_false(is.na(meta$start_datetime))
  expect_equal(meta$unit_time, expected_unit_time_with_timestamps)
  expect_false(timestamp_column %in% names(result))
})

test_that("read_[format] handles missing timestamps", {
  result <- read_[format](path_without_timestamps)

  meta <- aniframe::get_metadata(result)
  expect_true(is.na(meta$start_datetime))
  expect_equal(meta$unit_time, expected_unit_time_without_timestamps)
})

test_that("read_[format] converts elapsed time correctly", {
  result <- read_[format](path_with_timestamps)
...
test_that("read_[format] handles empty data gracefully", {
  result <- read_[format](path_empty)
...
test_that("read_[format] output works with aniframe functions", {
  result <- read_[format](path_valid)
